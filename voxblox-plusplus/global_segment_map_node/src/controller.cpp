...
void Controller::processSegment(
  ...
  if (lookupTransform(from_frame, world_frame_,
    ...
    segments_to_integrate_.push_back(segment);//给integrateFrame()处理segments_to_integrate_内的segments
    ptcloud_timer.Stop();
...
void Controller::integrateFrame(ros::Time msg_timestamp) {
  ...
  if (use_label_propagation_) {
    ...
    integrator_->decideLabelPointClouds(&segments_to_integrate_,//给待融合的segments打上labels
  ...
  if (label_tsdf_integrator_config_.enable_icp) {
    ...
    T_Gicp_C =
        integrator_->getIcpRefined_T_G_C(T_G_C, point_cloud_all_segments_t);//icp优化位姿
  }

  {
    ...
    for (Segment* segment : segments_to_integrate_) {
      ...
      integrator_->integratePointCloud(segment->T_G_C_, segment->points_C_,//怎么融合segments点云？
...
void Controller::segmentPointCloudCallback(
  ...
  // integrated.
  // 由于来自同一帧的segments时间戳一致，若检测到新的一帧，则时间戳改变 
  if (received_first_message_ &&
      last_segment_msg_timestamp_ != segment_point_cloud_msg->header.stamp) {//接收新一帧
    if (segments_to_integrate_.size() > 0u) {
      integrateFrame(segment_point_cloud_msg->header.stamp);//融合label、color、tsdf
    } else {
  ...
  processSegment(segment_point_cloud_msg);//预处理待融合的segments
}
...
bool Controller::resetMapCallback(std_srvs::Empty::Request& /*request*/,   //空的srv类型
...
bool Controller::toggleIntegrationCallback(
    std_srvs::SetBool::Request& request,    //request:  bool data启动或关闭硬件
    std_srvs::SetBool::Response& response) {//response: bool success 标志硬件是否成功运行
                                            //          string message 运行信息
  if (request.data ^ integration_on_) {// ^异或的位操作，异号为1
